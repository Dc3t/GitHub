...
@iqthon.on(admin_cmd(pattern=r"plist ?(.*)", outgoing=True))
@iqthon.on(sudo_cmd(pattern=r"plist ?(.*)", allow_sudo=True))
...
@iqthon.on(admin_cmd(pattern=r"blist ?(.*)", outgoing=True))
@iqthon.on(sudo_cmd(pattern=r"blist ?(.*)", allow_sudo=True))
async def get_users(show):
    await show.delete()
    if not show.text[0].isalpha() and show.text[0] not in ("/"):
        if not show.is_group:
